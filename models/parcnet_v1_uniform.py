...
class gcc_ca_mf_block(nn.Module):
    def __init__(self,
        ...
        if global_dim > 0:
            ...
            self.global_conv = OversizeConv2d(global_dim, meta_kernel_size)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.local_dim > 0 and self.global_dim > 0:
            local_attn, global_attn = torch.split(x, [self.local_dim, self.global_dim], dim=1)
            local_attn = self.local_conv(local_attn)
            global_attn = self.global_conv(global_attn)
            x = torch.cat([local_attn, global_attn], dim=1)
...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[gcc_ca_mf_block(dim=dims[i],
                    meta_kernel_size=meta_kernel_sizes[i],
                    ratio=ratios[i]) for j in range(depths[i])]
            )
...
@register_model
def parcnet_v1_uniform_xt(pretrained=False, in_22k=False, **kwargs):
