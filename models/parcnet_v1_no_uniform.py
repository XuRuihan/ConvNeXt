...
class MB_Attention(nn.Module):
    def __init__(self, d_model, kernel_size, ratio=0.5):
        ...
        self.lkc = MB_LKC(d_model, kernel_size, ratio)
...
class MF_Attention(nn.Module):
    def __init__(self, d_model, kernel_size, ratio=0.5):
        ...
        self.proj_1 = nn.Conv2d(d_model, d_model, 1)
        self.activation = nn.GELU()
        self.lkc = MF_LKC(d_model, kernel_size, ratio)
        self.proj_2 = nn.Conv2d(d_model, d_model, 1)
...
class MB_Block(nn.Module):
    def __init__(self, dim, mlp_ratio=4.0, drop=0.0, drop_path=0.0, act_layer=nn.GELU, kernel_size=32, ratio=0.5):
        ...
        self.attn = MB_Attention(dim, kernel_size, ratio)
...
class MF_Block(nn.Module):
    def __init__(self, dim, mlp_ratio=4.0, drop=0.0, drop_path=0.0, act_layer=nn.GELU, kernel_size=32, ratio=0.5):
        super().__init__()
        self.norm1 = nn.BatchNorm2d(dim)
        self.attn = MF_Attention(dim, kernel_size, ratio)
        self.drop_path = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        for i in range(4):
            if i <= 1:
                stage = nn.Sequential(
                    *[MB_Block(dim=dims[i], drop_path=dp_rates[cur],kernel_size=meta_kernel_sizes[i], ratio=ratios[i]) for j in range(depths[i])]
                )
            else:
                stage = nn.Sequential(
                    *[MF_Block(dim=dims[i], drop_path=dp_rates[cur],kernel_size=meta_kernel_sizes[i], ratio=ratios[i]) for j in range(depths[i])]
                )
...
@register_model
def parcnet_v1_no_uniform_xt(pretrained=False, in_22k=False, **kwargs):
